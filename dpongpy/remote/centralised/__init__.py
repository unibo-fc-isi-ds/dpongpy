...
class PongCoordinator(PongGame):
    ...
    def create_view(coordinator):
        ...
        class SendToPeersPongView(ShowNothingPongView):
            def render(self):
                event = coordinator.controller.create_event(ControlEvent.TIME_ELAPSED, dt=coordinator.dt,
                                                            status=self._pong)
            ...
            def on_time_elapsed(self, pong: Pong, dt: float, status: Pong = None):
                with terminal._lock:
                    if status is not None:
                        logger.debug(f"Reconciling with coordinator state (updates: {status.updates})")
                        pong.override(status)
                    else:
                        logger.debug(f"Speculative update (local updates: {pong.updates + 1})")
                        pong.update(dt)
    ...
    def _handle_ingoing_messages(self):
        while self.running:
            try:
                ...
                logger.debug("Received invalid message (not a pygame.event.Event)")

...
def udp_socket(bind_to: Address | int = Address.any_local_port(), stop: float | None = None) -> socket.socket:
    if isinstance(bind_to, int):
        bind_to = Address.localhost(bind_to)
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    if stop is not None:
        sock.settimeout(stop)
...
class UdpClient(UdpSession):
    def __init__(self, remote_address: Address):
        super().__init__(udp_socket(stop=0.01), remote_address)
